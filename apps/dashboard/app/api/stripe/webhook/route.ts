...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  const {
    business_id, customer_name, customer_phone, customer_email, items, source,
    fulfillment_type, delivery_address, delivery_fee, delivery_tier,
  } = session.metadata ?? {};
  ...
  // Build order record — include delivery fields if present
  ...
  if (fulfillment_type === "delivery") {
    if (delivery_address) orderRecord.ship_to      = { address: delivery_address };
    if (delivery_fee)     orderRecord.delivery_fee = parseFloat(delivery_fee);
  }

  const { data: newOrder } = await db.from("orders").insert(orderRecord).select("id").single();
